...
DOCKER_BUILD_SCRIPT=build_zlib_linux.sh
...
then
    echo "Missing argument 2: Base Docker image"
    exit 1
fi
...
if [ "${BASE_DOCKER_TAG}" == "" ]
then
    echo "Missing argument 3: Base Docker tag"
    exit 1
fi
...
# public.ecr.aws/ubuntu/ubuntu:18.04_stable
...
CMD_DOCKER_BUILD="docker build --build-arg INPUT_DOCKER_BUILD_SCRIPT=${DOCKER_BUILD_SCRIPT}\
 --build-arg INPUT_BASE_DOCKER_IMAGE=${DOCKER_INPUT_ARCHITECTURE}/${BASE_DOCKER_IMAGE} \
 --build-arg INPUT_BASE_DOCKER_TAG=${BASE_DOCKER_TAG} \
