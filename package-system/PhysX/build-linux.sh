...
DOCKER_BUILD_SCRIPT=docker_build_physx.sh
TARGET_BUILD_FOLDER=build
CUSTOM_TRIPLET=x64-linux-shared.cmake
...
# Copy over the custom triplet for shared libraries for vcpkg
cp $(pwd)/../../Scripts/vcpkg/triplets/${CUSTOM_TRIPLET} ${TEMP_FOLDER}/

echo "Building on ubuntu ${DOCKER_INPUT_ARCHITECTURE}/ubuntu:${UBUNTU_BASE}"
